...
# Creates a `Glint::Vector2` from a `Raylib::Vector2`.
def vec(v : Raylib::Vector2)
  Glint::Vector2.new(v)
end

# Creates a `Glint::Vector2` from x/y coordinates.
def vec(x : Number = 0, y : Number = 0)
  Glint::Vector2.new(x, y)
end

# Creates a `Glint::Vector2` from another `Glint::Vector2`.
def vec(v : Glint::Vector2)
  v.clone
end

# A `Glint::Position` is a `Glint::Vector2`.
alias Glint::Position = Glint::Vector2

# Creates a `Glint::Position` from x and y coordinates.
def p(x, y)
  Glint::Position.new(x, y)
end

# Creates a `Position` from a `Glint::Vector2`.
def p(v : Glint::Vector2)
  Glint::Position.new(v)
end

# A `Glint::Dimension` is a `Glint::Vector2`.
alias Glint::Dimension = Glint::Vector2

# Creates a `Dimension` from x and y coordinates.
def size(x, y)
  Glint::Dimension.new(x, y)
end

# Creates a `Dimension` from a `Vector2`.
def size(v : Glint::Vector2)
  Glint::Dimension.new(v)
end
